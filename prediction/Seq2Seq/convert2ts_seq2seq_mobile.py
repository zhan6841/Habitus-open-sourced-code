...
def convert2ts_seq2seq_10fold(FLAGS, LOG_DIR, features_list, labels_list, MAX_EPOCH, EPOCH_STEP):
    EVAL_DIR = os.path.join(LOG_DIR, 'ts_models_mobile')
    ...
    for train_index, test_index in kf.split(X):
        ...
        for model_save_path in to_eval_models:
            ...
            ts_model_save_path = os.path.join(EVAL_PRED_DIR, '%s.ptl' % (model_name))
            myModel.save_model_in_torch_script_mobile(test_loader, ts_model_save_path)
    ...
    for model_save_path in to_eval_models:
        ...
        for i in range(0, len(scenarios_included)):
            for j in range(0, 3):
                ...
                ts_model_save_path = os.path.join(EVAL_DIR, '%s.ptl' % (model_name))
                myModel.save_model_in_torch_script_mobile(test_loader, ts_model_save_path)
                print('Save torch script model to %s' % (ts_model_save_path))
                break
            break
